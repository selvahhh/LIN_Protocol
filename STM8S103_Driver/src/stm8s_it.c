...
INTERRUPT_HANDLER(EXTI_PORTC_IRQHandler, 5)
{
  ...
  */
    //uint8_t u8Pd3Status;
    //GPIOC->CR2 &= 0x7f;                  //PC7 intput Interrupt disabled
    
    if((GPIOC->IDR &0x80) == 0x00)
    {
      mode.modePc7 = 1;
      //mode.modePd3 = 0;
      //mode.modePd2 = 0;
      ...
      mode.modePc6 = 0;
      mode.modePc5 = 0;
      //bButtonControlTrigger = true;
      //mode.modePc5 = 0;
      //Mcu_vidPortIntDisable();
    }
    else if((GPIOC->IDR &0x40) == 0x00)
    {
      //if(mode.modePc6==1)
      //{
        //mode.modePc6 = 0;
    ...
    //delay_nms(50);                        //For 512 bytes data input from NFC, certain delay is needed
    //GPIOC->CR2 |= 0x80;                  //PC7 intput Interrupt eabled
...
INTERRUPT_HANDLER(EXTI_PORTD_IRQHandler, 6)
{
  /* In order to detect unexpected events during development,
     it is recommended to set a breakpoint on the following instruction.
  */        
    //uint8_t u8PortStatus;
    //GPIOD->CR2 &= 0xF7;                  //PD3 intput Interrupt disabled
    //TIM1->IER &=0xFC;
    //u8PortStatus = GPIOD->IDR &0x08;
    
    if((GPIOD->IDR &0x08) == 0x00)
    {
      mode.modePd3 = 1;
      //mode.modePd2 = 0;
      //mode.modePc7 = 0;
